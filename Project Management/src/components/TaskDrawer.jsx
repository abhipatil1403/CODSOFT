...
export default function TaskDrawer({ 
  ...
  // Keyboard listener inside drawer
  useEffect(() => {
  ...
  const handleAddSubtask = (e) => {
    ...
    if (newSubtaskTitle.trim()) {
      setSubtasks([
        ...subtasks,
        { id: `new-${Date.now()}`, title: newSubtaskTitle.trim(), completed: false }
      ]);
      setNewSubtaskTitle('');
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (title.length > 80) {
      setValidationError('Title cannot exceed 80 characters.');
      return;
    }
    if (dueDate) {
      ...
      if (selected < today) {
        setValidationError('Due date cannot be set in the past.');
        return;
      }
    }
    ...
    <div className="drawer-backdrop" onClick={onClose}>
      <div className="drawer-container glass-card" onClick={(e) => e.stopPropagation()}>
        <div className="drawer-header">
          ...
          <button className="btn-close-drawer" onClick={onClose} title="Close (Esc)">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="drawer-form">
          {userRole === 'member' && (
            <div className="restricted-banner">
              <span>⚠️ As a project member, you can only change task columns or check off subtasks.</span>
            </div>
          )}

          {validationError && (
            <div className="form-validation-error fade-in">
              {validationError}
            </div>
          )}
